...
class TestPrivacyPipeline:
    ...
    def setup_method(self):
        ...
        self.pipeline = PrivacyPipeline(
            storage_path=self.temp_dir, master_password="test_pipeline_password"
        )
        ...
        self.customer_data = pd.DataFrame(
            {
                ...
                "address": [
                    "Flat 5A, Nathan Rd, TST",
                    "Unit 10B, Queen Rd, Central",
                    "15樓A室, 旺角道, 旺角",
                ],
    ...
    def test_display_masking_toggle(self):
        ...
        display_result_masked = self.pipeline.retrieve_for_display(
            storage_key, privacy_enabled=True
        )
        assert display_result_masked.success is True
        masked_df = display_result_masked.display_data

        # Test with privacy disabled
        display_result_unmasked = self.pipeline.retrieve_for_display(
            storage_key, privacy_enabled=False
        )
    ...
    def test_non_pii_data_processing(self):
        ...
        pd.testing.assert_frame_equal(
            result.pseudonymized_data, self.non_pii_data, check_dtype=False
        )
...
class TestIntegrationScenarios:
    ...
    def test_telecom_customer_scenario(self):
        ...
        telecom_data = pd.DataFrame(
            {
                ...
                "address": [
                    "Flat 12A, 88 Nathan Road, Tsim Sha Tsui",
        ...
        display_result = self.pipeline.retrieve_for_display(
            result.storage_key, privacy_enabled=True
        )
        assert display_result.success is True
