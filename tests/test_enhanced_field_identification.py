...
class TestFieldType:
    ...
    def test_field_type_values(self):
        ...
        expected_types = [
            'account_id', 'hkid', 'email', 'phone', 'name', 'address',
            'passport', 'drivers_license', 'credit_card', 'bank_account',
            'ip_address', 'date_of_birth', 'general'
        ]
...
class TestFieldPattern:
    ...
    def test_field_pattern_creation(self):
        ...
        assert pattern.hong_kong_specific == False
...
class TestEnhancedFieldIdentifier:
    ...
    def test_default_patterns_loaded(self):
        ...
        expected_types = {
            FieldType.ACCOUNT_ID, FieldType.HKID, FieldType.EMAIL, FieldType.PHONE,
            FieldType.NAME, FieldType.ADDRESS, FieldType.PASSPORT, FieldType.DRIVERS_LICENSE,
            FieldType.CREDIT_CARD, FieldType.BANK_ACCOUNT, FieldType.IP_ADDRESS, FieldType.DATE_OF_BIRTH
        }
    ...
    def test_identify_field_address_hong_kong(self):
        ...
        result = identifier.identify_field("address", [
            "Flat 5A, 123 Nathan Road, Kowloon", 
            "Suite 1001, Tower 2, Hong Kong"
        ])
    ...
    def test_custom_patterns_loading(self):
        ...
        try:
            ...
            custom_patterns = [p for p in identifier.patterns 
                             if "CUSTOM" in str(p.value_patterns)]
...
class TestPerformanceAndAccuracy:
    ...
    def test_large_dataframe_performance(self):
        ...
        assert len(results) == 3
        assert results['Account_ID'].is_sensitive == True
        assert results['Email'].is_sensitive == True
        assert results['Plan_Type'].is_sensitive == False
    ...
    def test_confidence_scoring_accuracy(self):
        ...
        high_confidence_result = identifier.identify_field("email", 
            ["john@example.com", "jane@test.org", "bob@company.net"])
        
        # Test with low-confidence data  
        low_confidence_result = identifier.identify_field("data", 
            ["random", "text", "values"])
