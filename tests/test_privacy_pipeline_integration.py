...
class TestPrivacyPipelineIntegration:
    ...
    def setup_method(self):
        ...
        self.customer_data = pd.DataFrame({
            ...
            'address': ['Flat 12A, 88 Nathan Road, TST', 'Unit 5B, 123 Queen\'s Road, Central', '15樓C室, 456 彌敦道, 旺角'],
    ...
    def test_purchase_data_pipeline_integration(self):
        ...
        # Verify structure
        assert all(key in processed_data for key in [
            'original_data', 'pseudonymized_data', 'display_data', 
    ...
    def test_pseudonymized_data_has_no_original_pii(self):
        ...
        for field in pii_fields:
            if field in original_df.columns and field in pseudonymized_df.columns:
                ...
                assert len(overlap) == 0, f"Field {field} still contains original PII values: {overlap}"
    ...
    def test_metadata_completeness(self):
        ...
        required_fields = [
            'storage_key', 'pii_fields_identified', 'identification_results',
            'processing_stats', 'compliance'
        ]
    ...
    def test_hong_kong_specific_pii_detection(self):
        ...
        for field in hk_specific_fields:
            if field in identification_results:
                ...
                assert result['is_sensitive'] is True, f"HK-specific field {field} should be identified as sensitive"
                assert result['confidence'] > 0.5, f"HK-specific field {field} should have good confidence score"
