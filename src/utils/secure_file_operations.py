...
class SecureFileOperations:
    ...
    def _set_windows_permissions(self, file_path: Path):
        ...
        try:
            ...
            # Remove inheritance
            subprocess.run(["icacls", str(file_path), "/inheritance:r"], check=True, capture_output=True, text=True)

            # Grant full control to current user
            subprocess.run(
                ["icacls", str(file_path), f"/grant:r", f"{current_user}:F"], check=True, capture_output=True, text=True
            )
    ...
    def atomic_write(
        self, file_path: Union[str, Path], data: bytes, session_id: Optional[str] = None
    ) -> FileOperationResult:
        """
        ...
        try:
            ...
            try:
                with tempfile.NamedTemporaryFile(
                    dir=validated_path.parent, delete=False, prefix=f".tmp_{validated_path.name}_"
                ) as temp_file:
                ...
                return FileOperationResult(
                    success=True, message="File written successfully", path=validated_path, bytes_processed=len(data)
                )
            ...
            return FileOperationResult(success=False, message=f"Write failed: {str(e)}")

    def secure_read(self, file_path: Union[str, Path], session_id: Optional[str] = None) -> FileOperationResult:
        """
        ...
        try:
            ...
            if not validated_path.is_file():
                return FileOperationResult(success=False, message="Path is not a file")

            # Check file permissions
            if not os.access(validated_path, os.R_OK):
                return FileOperationResult(success=False, message="File not readable")
            ...
            return FileOperationResult(
                success=True, message="File read successfully", path=validated_path, bytes_processed=len(file_data)
            )

        except Exception as e:
            logger.error(f"Secure read failed for {file_path}: {e}")
            return FileOperationResult(success=False, message=f"Read failed: {str(e)}")

    def secure_delete(
        self, file_path: Union[str, Path], overwrite_passes: int = 3, session_id: Optional[str] = None
    ) -> FileOperationResult:
        """
        ...
        try:
            ...
            return FileOperationResult(success=False, message=f"Delete failed: {str(e)}")
    ...
    def list_directory(self, directory_path: Union[str, Path], session_id: Optional[str] = None) -> FileOperationResult:
        """
        ...
        try:
            ...
            if not validated_path.exists():
                return FileOperationResult(success=False, message="Directory not found")

            if not validated_path.is_dir():
                return FileOperationResult(success=False, message="Path is not a directory")

            # List files
            file_list = []
            for item in validated_path.iterdir():
                if item.is_file():
                    file_list.append({"name": item.name, "size": item.stat().st_size, "modified": item.stat().st_mtime})

            logger.info(f"Directory listed: {validated_path} ({len(file_list)} files)")

            return FileOperationResult(
                success=True, message=f"Directory listed successfully ({len(file_list)} files)", path=validated_path
            )

        except Exception as e:
            logger.error(f"Directory listing failed for {directory_path}: {e}")
            return FileOperationResult(success=False, message=f"Listing failed: {str(e)}")

    def file_exists(self, file_path: Union[str, Path], session_id: Optional[str] = None) -> bool:
        """
    ...
    def get_file_info(self, file_path: Union[str, Path], session_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
    ...
    def cleanup_directory(
        self, directory_path: Union[str, Path], session_id: Optional[str] = None, overwrite_passes: int = 0
    ) -> FileOperationResult:
        """
        ...
        try:
            ...
            if not validated_path.exists() or not validated_path.is_dir():
                return FileOperationResult(success=False, message="Directory not found or not a directory")
            ...
            return FileOperationResult(success=False, message=f"Cleanup failed: {str(e)}")
...
def secure_read_file(
    file_path: Union[str, Path], base_directory: Optional[Union[str, Path]] = None, session_id: Optional[str] = None
) -> FileOperationResult:
