...
class APILogger:
    ...
    def log_request(
        self, method: str, url: str, headers: Dict[str, str], payload: Dict[str, Any], request_id: Optional[str] = None
    ) -> str:
        """
        Log an API request.
        ...
        self.api_logger.info(
            f"REQUEST {request_id}: {method} {url} | Model: {model} | Prompt length: {len(prompt)} | Max tokens: {payload.get('max_tokens')}"
        )
    ...
    def log_rate_limit_event(
        self, event_type: str, identifier: str, requests_in_window: int, max_requests: int, wait_time: float = 0.0
    ) -> None:
        """
        ...
        if event_type == "blocked":
            ...
            self.api_logger.info(f"RATE_LIMIT {identifier}: Waiting {wait_time:.1f}s")
