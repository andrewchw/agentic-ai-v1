...
class EnhancedFieldIdentifier:
    ...
    def _load_default_patterns(self) -> List[FieldPattern]:
        ...
        return [
            ...
            FieldPattern(
                ...
                column_keywords=['name', 'customer', 'person', 'contact', 'first_name', 'last_name', 'full_name'],
                value_patterns=[
    ...
    def get_field_summary(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        hk_specific_count = sum(1 for result in analysis.values() 
                               if result.field_type in [FieldType.HKID, FieldType.ADDRESS] 
                               and result.is_sensitive)
...
def analyze_dataframe_fields(df: pd.DataFrame, config_path: Optional[str] = None) -> Dict[str, FieldIdentificationResult]:
