...
class DataMerger:
    ...
    def merge_datasets(
        ...
        try:
            ...
            logger.info(f"Merged {quality_report.matched_records} records with quality score {quality_report.quality_score:.2f}")
    ...
    def _generate_quality_report(self, customer_df: pd.DataFrame, purchase_df: pd.DataFrame) -> DataQualityReport:
        ...
        customer_duplicates = customer_df[customer_df[self.account_id_column].duplicated()][self.account_id_column].tolist()
        purchase_duplicates = purchase_df[purchase_df[self.account_id_column].duplicated()][self.account_id_column].tolist()
        ...
        if total_unique_ids > 0:
            ...
            duplicate_penalty = (len(customer_duplicates) + len(purchase_duplicates)) / (len(customer_df) + len(purchase_df))
    ...
    def _perform_merge(self, customer_df: pd.DataFrame, purchase_df: pd.DataFrame, strategy: MergeStrategy) -> Dict[str, Any]:
        ...
        try:
            ...
            customer_clean = customer_clean.add_prefix('customer_').rename(columns={f'customer_{self.account_id_column}': self.account_id_column})
            purchase_clean = purchase_clean.add_prefix('purchase_').rename(columns={f'purchase_{self.account_id_column}': self.account_id_column})
