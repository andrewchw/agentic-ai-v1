    ...
    return pd.DataFrame(
        {
            ...
            "Email": [
                ...
                "wong.taixin@gmail.com",
                "alice.chen@outlook.com",
            ...
            "Address": [
                ...
                "789 Pine Road, New Territories",
                "Room 15B, 321 Queen's Road, Central",
...
def demonstrate_enhanced_field_identification():
    ...
    print("-" * 80)
    print(
        f"{'Column Name':<20} {'Field Type':<15} {'Confidence':<12} {'Sensitive':<10} {'Method':<15}"
    )
    print("-" * 80)

    for column, result in analysis_results.items():
        print(
            f"{column:<20} {result.field_type.value:<15} {result.confidence:<12.2f} "
    ...
    hk_test_data = {
        ...
        "HK_Address": [
            "Flat 10A, 123 Nathan Road, Kowloon",
    ...
    for field_name, values in confidence_test_cases:
        result = identifier.identify_field(field_name, values)
        print(
            f"  {field_name}: {result.field_type.value} (confidence: {result.confidence:.2f}, "
    ...
    print(
        f"  Sensitive columns identified: {len([r for r in large_analysis.values() if r.is_sensitive])}"
    )
