...
def demonstrate_privacy_integration():
    ...
    try:
        ...
        if customer_success:
            ...
            if stats:
                ...
                print(f"   📊 Fields processed: {stats.get('pii_fields_identified', 0)} PII, {stats.get('pii_fields_masked', 0)} masked")
            
            # Show compliance status
            compliance = metadata.get('compliance', {})
            if compliance:
                print(f"   ✅ GDPR compliant: {compliance.get('gdpr_compliant', False)}")
                print(f"   ✅ Hong Kong PDPO compliant: {compliance.get('hong_kong_pdpo_compliant', False)}")
                print(f"   🔒 Original data encrypted: {compliance.get('original_data_encrypted', False)}")
                print(f"   🚫 No external PII transmission: {compliance.get('no_external_pii_transmission', False)}")
        else:
    ...
    for field in pii_fields:
        if field in original_customer.columns and field in pseudo_customer.columns:
            ...
            if overlap:
                pii_in_pseudo = True
                print(f"   ❌ WARNING: Original PII found in pseudonymized data for field {field}: {overlap}")
