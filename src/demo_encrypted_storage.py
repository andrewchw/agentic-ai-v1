    ...
    return pd.DataFrame(
        {
            ...
            "address": [
                "Flat 5A, 123 Nathan Rd, TST",
                "Unit 10B, 456 Queen Rd, Central",
                "Shop 7, 789 Hennessy Rd, WC",
                "15樓A室, 100旺角道, 旺角",
                "20樓B室, 200銅鑼灣道, 銅鑼灣",
            ],
...
def demonstrate_dataframe_encryption():
    ...
    print(
        f"   Shape preservation: {shapes_match} (Original: {pii_df.shape}, Retrieved: {retrieved_df.shape})"
    )
    ...
    for idx in range(min(3, len(pii_df))):
        ...
        print(
            f"   Row {idx}: Name match: {original_name == retrieved_name}, HKID match: {original_hkid == retrieved_hkid}"
        )
...
def demonstrate_security_features():
    ...
    print(
        f"   ✅ Derivation time: {key_derivation_time:.3f} seconds (indicates sufficient iterations)"
    )
...
def main():
    ...
    try:
        ...
        demonstrate_dataframe_encryption()
